...
form.addEventListener("submit", function (event) {
  ...
  if (selectedCampuses.length === 0) {
    ...
    // Let the user know if they chose "one" preference but selected more than one campus
  if (type === "one" && selectedCampuses.length > 1) {
    output.textContent = "Select only one campus.";
    return;
  }


  // Let the user know if they choose many campuses but didn't put a note that they need to add a note
  if (type === "many" && selectedCampuses.length >= 2 && note === "") {
    output.textContent = "Add a note.";
    return;
  }
